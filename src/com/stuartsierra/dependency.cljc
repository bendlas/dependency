...
(defn- transitive
  ...
  ([neighbors node-set] (transitive neighbors node-set #{}))
  ([neighbors node-set expanded]
   (if (pos? (count node-set))
     (let [sub-node-set (into #{}
                              (comp (mapcat neighbors)
                                    (remove expanded))
                              node-set)]
       (recur neighbors sub-node-set (into expanded sub-node-set)))
     expanded)))
